...
router.post(
	...
	[
		...
		body('idNumber').notEmpty().withMessage('is required.').bail().isString().bail().custom(Validation.unique(Student, 'idNumber')),
		...
		body('email').notEmpty().withMessage('is required.').bail().isEmail().bail().custom(Validation.unique(Student, 'email')),
